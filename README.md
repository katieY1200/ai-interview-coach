...
| AI | Google Gemini API | 한국어 품질 우수, 무료 티어로 개발 가능 | OpenAI 대비 안정성 낮음; API 장애 시 직무별 예비 질문 풀로 대체 |
...
- **면접 부적절 행동 검출 확대** — 현재는 시선 이탈·과도한 움직임·무표정·얼굴 미감지만 잡고 있는데,
  머리카락 만지기(손이 얼굴 근처로 반복 이동), 턱 괴기(팔꿈치-얼굴 랜드마크 거리), 자세 흐트러짐(어깨 기울기 비대칭) 등
  면접에서 감점 요인이 되는 습관성 행동까지 감지 범위를 넓힐 예정

### 예외 처리 보강

현재 서비스의 에러 핸들링이 단순한 부분이 있어 보완 예정:

- **Gemini API 에러 세분화** — 현재는 모든 API 실패를 동일하게 fallback 처리함. Rate limit(429), 네트워크 타임아웃, 잘못된 응답 형식을 구분해 재시도 로직 또는 사용자 안내를 다르게 제공해야 함
...
- **AI 응답 JSON 파싱 실패** — Gemini가 JSON 형식이 아닌 텍스트를 반환하면 `_parse_json()`이 예외를 던짐. 현재는 `None` 반환 후 fallback 질문 풀로 대체하는데, 빈 커버레터나 비정상 입력에 대한 사전 검증을 앞단에서 처리하면 더 안정적
